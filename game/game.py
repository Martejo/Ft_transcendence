...
class PowerUpOrb:
    ...
    next_spawn_time = time.time() + random.uniform(5, 8)
    global_spawn_cooldown = random.uniform(5, 8)
    ...
    def __init__(self, terrain_rect, color, effect_type):
        ...
        self.respawn_time = random.uniform(5, 8)
...
while True:
    ...
    if ball.colliderect(paddle_left.rect.inflate(HITBOX_MARGIN, 0)):
    ...
    elif ball.colliderect(paddle_right.rect.inflate(HITBOX_MARGIN, 0)):
    ...
    for bumper in bumpers:
        ...
        if bounce:
            # Reverse and increase x speed (like paddles)
            ball_speed_x = -ball_speed_x * 1.1  # 10% increase
            # Calculate y speed based on relative position and x speed (like paddles)
