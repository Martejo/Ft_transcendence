...
class JWTAuthenticationMiddleware:
    ...
    def __call__(self, request):
        ...
        if auth_header:
            try:
                logger.debug("Test du jeton jwt")
                # Extrait le jeton
                token = auth_header.split(' ')[1]
                # Décode le jeton
                payload = jwt.decode(token, settings.SECRET_KEY, algorithms=['HS256'])
                # Le user_id est contenu dans le payload du JWT (on ne le conserve pas en base de données)
                ...
                logger.debug("Erreur de Jeton jwt")
                request.user = AnonymousUser()
        else:
            logger.debug("Pas de jwt")
            request.user = AnonymousUser()
